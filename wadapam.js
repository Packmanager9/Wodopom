...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Guy {
        constructor(tile) {
            ...
            this.count = 0
            this.type = Math.floor(Math.random() * 12)
            this.selected = 0
            this.target = {}
            this.path = [this.tile]
            this.step = 0
            ...
            this.stats = [4 + Math.floor(this.type * .5), 0, 0, 0, 0, 0]
            if (this.type == 7) {
                this.stats[0] = 2
            }
        ...
        draw() {
            ...
            // if (this.path[this.step].walkable == true || (this.tile == this.path[this.step])) {
    ...
    class Ship {
        constructor(width, height, posx, posy, gridPoints) {
            ...
            this.guys = [new Guy(tiles[0]), new Guy(tiles[1]), new Guy(tiles[2]), new Guy(tiles[3])]
        }
        ...
        draw() {
            ...
            // for(let t = 0;t<this.blocks.length;t++){
            //     for(let k = 0;k<this.blocks[t].length;k++){
            // //     // let link = new LineOP(this.body, new Point(this.tiles[t].x+(this.tiles[t].width*.5), this.tiles[t].y+(this.tiles[t].height*.5)))
            // //     // console.log(link)
            // //     if(this.blocks[t][k].marked == 1){
